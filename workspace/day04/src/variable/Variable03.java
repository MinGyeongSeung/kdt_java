...
//		int @!#@$%@#%$ = 100; // 특수기호로 선언시 오류발생
		int a$b_ = 110;
		int _ab$ = 120; // 가능하지만 1) 문자부터 시작한다를 생각한다!!
		System.out.println(a$b_);
		System.out.println(_ab$);
		
		//3) 공백을 사용할 수 없다(카멜표기법, 스네이크표기법, 파스칼표기법 등을 활용한다)
//		String user name = "민경승";
		String userName = "민경승";
		//첫단어 소문자로 시작, 단어가 바뀔 때 첫문자를 대문자로 작성(카멜표기법, 변수명/메소드명 작성시)
		String user_name = "민경승";
		// 첫 단어 소문자로 시작, 단어가 바뀔 때 언더바로 연결해서 작성(스네이크 표기법, 상수 작성시)
		String UserName = "민경승";
		// 첫단어의 첫문자를 대문자로 시작, 단어가 바뀔 때 첫 문자를 대문자로 작성(파스칼표기법, 클래스 명 작성시)
